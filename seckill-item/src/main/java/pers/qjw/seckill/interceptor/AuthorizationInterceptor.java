...
public class AuthorizationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        // 如果方法没有注明Authorization，则直接通过 ，抛出异常
        if (method.getAnnotation(Authorization.class) == null && method.getAnnotation(GetUserId.class) == null) {
            return true;
        } else {
            // 如果方法注明了Authorization 获取所有 cookie
        ...
        if (method.getAnnotation(Authorization.class) != null){
            throw new TokenException("401", "还未登录");
        } else {
            return true;
        }
    }
